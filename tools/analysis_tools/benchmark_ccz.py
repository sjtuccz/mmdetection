...
# python tools/analysis_tools/benchmark_ccz.py ccz/mask-rcnn_convnext-t-p4-w7_fpn_amp-ms-crop-3x_coco.py --task inference --launcher pytorch --max-iter 2000 --checkpoint checkpoint/mask_rcnn_convnext-t_p4_w7_fpn_fp16_ms-crop_3x_coco_20220426_154953-050731f4.pth
import argparse
import os
...
from mmdet.utils.benchmark import (DataLoaderBenchmark, DatasetBenchmark,
                                   InferenceBenchmark, TQ_InferenceBenchmark, TQ_InferenceBenchmark_excludeIO)
...
def parse_args():
    ...
    parser.add_argument(
        '--log-interval', type=int, default=200, help='interval of logging')
    parser.add_argument(
...
def inference_benchmark(args, cfg, distributed, logger):
    # if 'tq' in cfg.model.backbone.type.lower():
        # print('find tq............................................................')
    benchmark = TQ_InferenceBenchmark_excludeIO(
            cfg,
            checkpoint=None,  # 强制设为 None，不加载 checkpoint
            distributed=distributed,
            is_fuse_conv_bn=args.fuse_conv_bn,
...
def main():
    ...
    # ========== 修改点：强制使用单卡，不使用分布式 ==========
    # 无论用户传入什么 launcher，都强制设置为 'none'
    # 这样模型就不会被 DistributedDataParallel 包装
    args.launcher = 'none'
    distributed = False
    # =====================================================
